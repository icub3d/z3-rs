...
**Goal:** Demonstrate how `push` and `pop` isolate constraints.

**Logic:**
1.  Assert `x < 10` (Base).
2.  `push()` -> Enter Scope 1.
3.  Assert `x > 5`. Check (Expect SAT, e.g., x=6).
4.  `pop()` -> Exit Scope 1 (Undo `x > 5`).
5.  Assert `x == 2`. Check (Expect SAT, x=2). 
    *   *Note:* If we hadn't popped, `x > 5` AND `x == 2` would be UNSAT.
