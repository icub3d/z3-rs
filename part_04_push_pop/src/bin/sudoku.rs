...
impl SudokuSolver {
    ...
    fn push_assertion(&self, r: usize, c: usize, val: i64) {
        ...
        self.solver.assert(self.cells[r][c].eq(&z3_val));
    }
...
impl App {
    ...
    fn on_key(&mut self, key: KeyCode) -> bool {
        ...
        if self.state != GameState::Error {
             match key {
                ...
                KeyCode::Down | KeyCode::Char('j') => if self.cursor.0 < 3 { self.cursor.0 += 1 },
                KeyCode::Left | KeyCode::Char('h') => if self.cursor.1 > 0 { self.cursor.1 -= 1 },
                KeyCode::Right | KeyCode::Char('l') => if self.cursor.1 < 3 { self.cursor.1 += 1 },
                _ => {}
            }
        }
    ...
    fn undo_last_move(&mut self) {
        if let Some((r, c, _)) = self.history.pop() {
            ...
            match self.solver.check() {
                SatResult::Sat => self.state = GameState::Playing,
                SatResult::Unsat => self.state = GameState::Error, // Should not happen if history logic is sound
                _ => self.state = GameState::Playing,
            }
...
fn draw_ui(terminal: &mut Terminal<CrosstermBackend<Stdout>>, app: &App) -> io::Result<()> {
    terminal.draw(|f| {
        ...
        let status_text = match app.state {
            GameState::Playing => Span::styled("Status: Playing (SAT)", Style::default().fg(Color::Green)),
            GameState::Error => Span::styled("Status: UNSAT! Conflict.", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
            GameState::Solved => Span::styled("Status: SOLVED!", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
        };
        ...
        let help_text = vec![
            ...
            Line::from(Span::styled(controls_text, Style::default().fg(Color::Gray))),
        ];

        let info_block = Paragraph::new(help_text)
            .block(Block::default().borders(Borders::TOP).title(" Info ").style(Style::default().fg(Color::Blue)))
            .alignment(Alignment::Center);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        if event::poll(Duration::from_millis(100))? && let Event::Key(key) = event::read()? && app.on_key(key.code) {
                    break;
        }
    }
